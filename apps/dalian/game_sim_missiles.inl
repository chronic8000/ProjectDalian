...
if (input.launch_missile && state_.missile_reload <= 0.f) {
  state_.missile_reload = 1.4f;
  ...
  if (params_.world) {
    ...
    am.m.velocity = dir0 * 55.f;
    if (params_.missile_headless_demo) {
      am.m.position = eye;
      am.prev_pos = eye;
    ...
    if (std::getenv("BF2_MISSILE_DEBUG")) {
      std::fprintf(stderr, "[missile] LAUNCH pos=%.1f,%.1f,%.1f vel=%.1f,%.1f,%.1f\n",
                   am.m.position.x, am.m.position.y, am.m.position.z, am.m.velocity.x,
                   am.m.velocity.y, am.m.velocity.z);
    }
    state_.explosions.push_back({launch, 0.f, 0.25f});
    for (int i = 0; i < 6; ++i) {
      state_.smoke.push_back({launch - dir0 * (0.3f * static_cast<float>(i)), 0.f, 1.2f});
    }
  }
}
...
for (auto& am : state_.missiles) {
  ...
  if (am.smoke_timer <= 0.f) {
    ...
    if (state_.smoke.size() < 6000) {
      state_.smoke.push_back({am.m.position - am.m.forward() * 0.6f, 0.f, 1.5f});
    }
  }

  if (detonate) {
    am.m.alive = false;
    state_.explosions.push_back({boom, 0.f, 0.7f});
    explode_at(boom, 9.f, 150.f);
    for (int i = 0; i < 10; ++i) state_.smoke.push_back({boom, 0.f, 1.8f});
    if (std::getenv("BF2_MISSILE_DEBUG")) {
