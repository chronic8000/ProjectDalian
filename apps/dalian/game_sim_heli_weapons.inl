...
if (state_.in_vehicle < 0 || state_.in_vehicle >= static_cast<int>(state_.vehicles.size()) ||
    !state_.vehicles[state_.in_vehicle].is_air) {
...
if (has_gunner && (ai_gunning || player_gunning)) {
  ...
  auto visible = [&](int i, float& out_range) -> bool {
    ...
    if (d < 1e-3f || d > 200.f) return false;
  ...
  if (player_gunning) {
    ...
    if (input.fire && input.mouse_look && !input.deploy_open && state_.heli_gun_cd <= 0.f) {
      ...
      if (eh.idx >= 0) {
        ...
        state_.tracers.push_back({gun, gun + sd * 260.f, 0.05f});
      }
    }
    if (input.fire_secondary && state_.heli_grocket_cd <= 0.f) {
      state_.heli_grocket_cd = 1.2f;
      fire_heli_rocket(gun + nose * 1.0f, input.look_forward);
    }
  } else if (ai_ready) {
    ...
    if (state_.heli_gun_cd <= 0.f) {
      state_.heli_gun_cd = 0.11f;
      state_.gunner_engaging = true;
      const glm::vec3 sd = glm::normalize(
    ...
    if (state_.heli_grocket_cd <= 0.f && range < 160.f) {
      state_.heli_grocket_cd = 4.0f;
      fire_heli_rocket(gun + nose * 1.0f, dir);
    }
  }
} else {
  state_.gunner_target = -1;
